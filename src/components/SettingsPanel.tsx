...
function SettingsSection({ title, description, category, items, checkUsage }: SettingsSectionProps): React.JSX.Element {
...
export function SettingsPanel(): React.JSX.Element {
    const { settings, issues } = useAppContext();
    ...
    return (
        <div className='max-w-7xl mx-auto'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8'>
                <SettingsSection
                    title='Projects'
                    description='Manage the list of projects available for issues.'
                    category='projects'
                    items={settings.projects}
                    checkUsage={(item) => issues.some((i) => i.projectName === item)}
                />

                <SettingsSection
                    title='Team Members'
                    description='People who can report or be assigned to issues.'
                    category='people'
                    items={settings.people}
                    checkUsage={(item) => issues.some((i) => i.reporter === item || i.assignees.includes(item))}
                />

                <SettingsSection
                    title='Issue Types'
                    description='Categorize issues (e.g., Bug, Feature).'
                    category='issueTypes'
                    items={settings.issueTypes}
                    checkUsage={(item) => issues.some((i) => i.type === item)}
                />

                <SettingsSection
                    title='Priorities'
                    description='Define priority levels for issues.'
                    category='priorities'
                    items={settings.priorities}
                    checkUsage={(item) => issues.some((i) => i.priority === item)}
                />

                <SettingsSection
                    title='Statuses'
                    description='Workflow stages for tracking progress.'
                    category='statuses'
                    items={settings.statuses}
                    checkUsage={(item) => issues.some((i) => i.status === item)}
                />
            </div>
        </div>
    );
